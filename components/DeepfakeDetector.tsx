...
const DeepfakeDetector: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    if (!videoFile) {
      setError('Please select a video file to analyze for deepfake patterns.');
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      setLoadingMessage('Analyzing video frames for deepfake artifacts...');
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <FileInput
        ...
        label="Video File for Deepfake Analysis"
        disabled={isLoading}
      />
